...
class ImplicitModel(nn.Module):
    ...
    def _generate_with_insertion(
        ...
        if insert_const_ids_in_cot and insert_position > 0:
            ...
            if self._check_double_eos_in_output(beam_output, input_ids):
    ...
    def _check_double_eos_in_output(self, output_ids, input_ids):
        # If we have 2 or more EOS tokens, we're done (one after CoT, one after answer)
        output_eos_count = (output_ids == self.tokenizer.eos_token_id).sum(dim=-1)
        input_eos_count = (input_ids == self.tokenizer.eos_token_id).sum(dim=-1)
        eos_count = output_eos_count - input_eos_count
    ...
    def reinit_processor_criteria(self, logits_processor, stopping_criteria, single=False):
        if logits_processor is None:
            return None, None
        ...
        if single:
            ...
            logits_processor = SingleEOSLogitsProcessor(self.tokenizer.eos_token_id)
            stopping_criteria = SingleEOSStoppingCriteria(self.tokenizer.eos_token_id)
